...
| **Unhappy Case** (lỗi do code) | Lỗi phát hiện sau khi đã Merge vào nhánh chính | Sau bước Quality Gate, code đã vào `main` | Dùng lệnh đảo ngược (git revert) tạo commit mới đảo ngược, không dùng lệnh xóa lịch sử (git reset), chạy lại pipeline |
| **Unhappy Case** (lỗi do code) | Sự cố Production sau khi Deploy | Sau bước Deploy Production | Rollback khẩn cấp: re-deploy Artifact/Image cũ, hoặc chuyển luồng truy cập theo mô hình Blue/Green |
| **Trường hợp bất ngờ** (do hạ tầng/vận hành, không do mã nguồn) | Pipeline bị gián đoạn (hết thời gian chờ, máy chạy tự động gặp sự cố, hạ tầng bên thứ ba gặp sự cố, thông tin xác thực hết hạn...) | Bất kỳ bước nào | Auto-Retry có giới hạn, báo cho đội vận hành hệ thống, chạy lại pipeline mà không cần sửa mã nguồn |
| **Trường hợp thành công mở rộng** | Triển khai thành công nhưng chủ động khôi phục lại bản cũ vì lý do kinh doanh/trải nghiệm người dùng, không phải lỗi | Sau bước triển khai lên môi trường sản xuất, hệ thống vẫn ổn định | Triển khai lại bản cũ theo kế hoạch, cần phê duyệt thủ công trước, không phải xử lý khẩn cấp |
